  ...
  def build_menu(parent_id=nil)
    ...
    unless children.empty?
      ...
      for child in children
        ...
        concat image_tag("#{move_loc}", :class => "icon handle") + ' ' if children.size > 1 and !child.parent_id.blank?
        concat child.updated_at.strftime("%m/%d/%Y &mdash; &nbsp;")
        if MATERIALS.include?(child.permalink)
          concat h(child.title)
        else
        ...
        concat ' ' + icon("Write", edit_admin_page_path(child)) unless MATERIALS.include?(child.permalink)
        if child.can_delete
        ...
        concat icon("Picture", [:admin, child]) + ' ' + link_to(child.images_count, [:admin, child])
        concat "</div>" + clear
        build_menu(child.id)
        concat "</li>\n"
      end
