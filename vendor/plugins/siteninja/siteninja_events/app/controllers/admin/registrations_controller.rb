class Admin::EventRegistrationsController < AdminController
  ...
  def index
    ...
    @registration_contacts = RegistrationContact.find(:all, :conditions => { :registration_id => approved_registrations.collect(&:id) }, :include => :contact, :include => :event_price)
  end
  ...
  def csv
    ...
    registration_contacts = RegistrationContact.find_all_by_registration_id(registrations.collect(&:id), :include => :contact, :order => "contacts.last_name, contacts.first_name")
